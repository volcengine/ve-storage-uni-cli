...
"""Black-box E2E coverage for ``ve-tos inventory`` low-level APIs.
...
def inventory_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_inventory_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    inventory_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-INVENTORY-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in inventory_leaf_commands:
    ...
    assert total_cases >= len(inventory_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_inventory_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    inventory_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-INVENTORY-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in inventory_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_inventory_set_get_list_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-INVENTORY-LIVE set -> get -> list -> delete -> get."""

    account_id = os.environ.get("TOS_E2E_INVENTORY_ACCOUNT_ID")
    role = os.environ.get("TOS_E2E_INVENTORY_ROLE")
    if not account_id or not role:
        pytest.skip("set TOS_E2E_INVENTORY_ACCOUNT_ID and TOS_E2E_INVENTORY_ROLE")

    inventory_id = "ve-tos-cli-e2e-inventory"
    destination_bucket = os.environ.get("TOS_E2E_INVENTORY_BUCKET", e2e_bucket_name)
    config = {
        "Id": inventory_id,
        "IsEnabled": False,
        "Destination": {
            "TOSBucketDestination": {
                ...
                "Prefix": "ve-tos-cli-e2e-inventory",
            }
        },
    ...
    try:
        live_bucket_config.set_inventory(e2e_bucket_name, inventory_id, config)

        get_result = cli_runner.run(
            ["ve-tos", "inventory", "get", "--bucket", e2e_bucket_name, "--id", inventory_id]
        )
        ...
        assert body["id"] == inventory_id
        assert body["is_enabled"] is False

        list_result = cli_runner.run(["ve-tos", "inventory", "list", "--bucket", e2e_bucket_name])
        assert list_result.exit_code == 0, list_result.stderr
        envelope_assert.assert_success_envelope(list_result.require_envelope())
        assert inventory_id in json.dumps(list_result.payload(), sort_keys=True)

        live_bucket_config.cleanup_inventory(e2e_bucket_name, inventory_id)

        deleted_get = cli_runner.run(
            ["ve-tos", "inventory", "get", "--bucket", e2e_bucket_name, "--id", inventory_id]
        )
