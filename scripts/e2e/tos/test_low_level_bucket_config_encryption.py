...
"""Black-box E2E coverage for ``ve-tos encryption`` low-level APIs.
...
def encryption_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_encryption_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    encryption_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ENCRYPTION-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in encryption_leaf_commands:
    ...
    assert total_cases >= len(encryption_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_encryption_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    encryption_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ENCRYPTION-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in encryption_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_encryption_set_get_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-ENCRYPTION-LIVE set -> get -> delete -> get with restoration."""
    ...
    if not https_endpoint:
        pytest.skip("TOS_HTTPS_ENDPOINT not set; encryption requires HTTPS")
    assert isinstance(https_endpoint, str)

    extra_env: dict[str, str] = {"TOS_ENDPOINT": https_endpoint}

    probe = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", e2e_bucket_name], extra_env=extra_env)
    if probe.exit_code != 0 and probe.exit_code != envelope_assert.EXIT_CODES["resource_not_found"]:
        skip_on_live_error("ve-tos encryption get (probe)", probe)
    original_config = None
    if probe.exit_code == 0:
        envelope_assert.assert_success_envelope(probe.require_envelope())
        original_config = probe.payload().get("body")
    ...
    try:
        ...
        if set_result.exit_code != 0:
            skip_on_live_error("ve-tos encryption set", set_result)
        envelope_assert.assert_success_envelope(set_result.require_envelope())

        get_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", e2e_bucket_name], extra_env=extra_env)
        if get_result.exit_code != 0:
            skip_on_live_error("ve-tos encryption get", get_result)
        envelope_assert.assert_success_envelope(get_result.require_envelope())
        body = get_result.payload()["body"]
        rule = body.get("rule") or body.get("Rule")
        if not rule:
            # Fallback: some responses wrap in rules array
            rules = body.get("rules") or body.get("Rules")
            assert rules, f"expected encryption rule in body, got {body!r}"
            rule = rules[0]
        default_enc = (
            rule.get("apply_server_side_encryption_by_default")
            or rule.get("ApplyServerSideEncryptionByDefault")
        )
        algo = default_enc.get("sse_algorithm") or default_enc.get("SSEAlgorithm")
        assert algo == "AES256", f"expected AES256, got {algo!r}"

        delete_result = cli_runner.run(
            [
                "ve-tos", ROOT, "delete", "--bucket", e2e_bucket_name,
                "--force", "--confirm", f"tos://{e2e_bucket_name}",
            ],
            extra_env=extra_env,
        )
        if delete_result.exit_code != 0:
            skip_on_live_error("ve-tos encryption delete", delete_result)
        envelope_assert.assert_success_envelope(delete_result.require_envelope())

        deleted_get = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", e2e_bucket_name], extra_env=extra_env)
