...
"""Black-box E2E coverage for ``ve-tos tagging`` low-level APIs.
...
def tagging_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_tagging_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    tagging_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-TAGGING-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in tagging_leaf_commands:
    ...
    assert total_cases >= len(tagging_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_tagging_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    tagging_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-TAGGING-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in tagging_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_tagging_set_get_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-TAGGING-LIVE set -> get -> delete -> get with restoration."""
    ...
    try:
        ...
        tag = get_result.payload()["body"]["tag_set"]["tags"][0]
        assert tag["key"] == "ve-tos-cli-e2e"
        assert tag["value"] == "tagging"
