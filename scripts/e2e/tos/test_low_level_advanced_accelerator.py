...
"""Black-box E2E coverage for ``ve-tos accelerator`` low-level APIs.
...
def accelerator_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_accelerator_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    accelerator_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-ACCELERATOR-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in accelerator_leaf_commands:
    ...
    assert total_cases >= len(accelerator_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_accelerator_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    accelerator_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-ACCELERATOR-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in accelerator_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_accelerator_control_plane_list_live_chain(
    cli_runner: CliRunner,
) -> None:
    """@case-id LL-ADV-ACCELERATOR-LIVE control endpoint list probe."""

    account_id = os.environ.get("TOS_ACCOUNT_ID")
    if not account_id:
        pytest.skip("TOS_ACCOUNT_ID not set; control-plane commands require account ID")

    result = cli_runner.run(["ve-tos", 'accelerator', 'list', "--account-id", account_id, "--output", "json"])
    if result.exit_code != 0:
        skip_on_live_error("ve-tos accelerator list", result)
    envelope_assert.assert_success_envelope(result.require_envelope())
    assert isinstance(result.payload(), dict)


@pytest.mark.destructive
def test_accelerator_create_get_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-ADV-ACCELERATOR-CRUD create -> get -> list -> bind -> unbind -> delete cycle."""
    ...
    if not tos_region:
        pytest.skip("TOS_REGION not set; accelerator create E2E requires region")

    accelerator_name = f"e2etest_{e2e_bucket_name[8:16].replace('-', '')}"
    accelerator_id = None

    # Step 1: List AZs to find a valid region/az
    az_result = cli_runner.run([
        "ve-tos", ROOT, "list-az", "--account-id", account_id,
        "--region", tos_region, "--output", "json",
    ])
    az_info = ""
    if az_result.exit_code != 0:
        skip_on_live_error("ve-tos accelerator list-az", az_result)
    body = az_result.payload().get("body", az_result.payload())
    azs = body.get("azs") or body.get("Azs") or body.get("AvailableZones") or []
    if azs and isinstance(azs, list):
        first_az = azs[0]
        az_info = (
            first_az
            if isinstance(first_az, str)
            else (
                first_az.get("az")
                or first_az.get("Az")
                or first_az.get("ZoneId")
                or first_az.get("Id")
                or ""
            )
        )
    if not az_info:
        # [Review Fix #AcceleratorAz] CreateAccelerator requires a real AZ from
        # the same TOS_REGION under test; falling through creates a misleading
        # "missing az" service error.
        pytest.skip(f"No accelerator AZ available for region {tos_region}")

    create_config = {
        "Name": accelerator_name,
        "Description": "E2E test accelerator",
    }
    if az_info:
        create_config["Az"] = az_info
    ...
    try:
        ...
        body = create_result.payload().get("body", create_result.payload())
        accelerator_id = (
            body.get("accelerator_id") or body.get("AcceleratorId")
            or body.get("id") or body.get("Id") or ""
        )

        if accelerator_id:
            # Step 3: Get accelerator
            get_result = cli_runner.run([
                "ve-tos", ROOT, "get", "--account-id", account_id,
                "--id", accelerator_id, "--output", "json",
            ])
