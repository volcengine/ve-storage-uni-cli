...
"""Black-box E2E coverage for ``ve-tos access-monitor`` low-level APIs.
...
def access_monitor_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_access_monitor_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    access_monitor_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ACCESS_MONITOR-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in access_monitor_leaf_commands:
    ...
    assert total_cases >= len(access_monitor_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_access_monitor_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    access_monitor_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ACCESS_MONITOR-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in access_monitor_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_access_monitor_set_get_restore_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-ACCESS-MONITOR-LIVE get -> set -> get -> restore."""

    original_config = live_bucket_config.get_required(ROOT, e2e_bucket_name)

    try:
        live_bucket_config.set_config(ROOT, e2e_bucket_name, {"Status": "Disabled"})
        ...
        assert get_result.payload()["body"]["status"] == "Disabled"
    finally:
        live_bucket_config.restore_optional(
            ROOT,
            e2e_bucket_name,
            original_config,
            normalize=lambda config: {"Status": config["status"]},
        )
