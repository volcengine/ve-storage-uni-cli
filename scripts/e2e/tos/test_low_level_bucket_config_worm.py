...
"""Black-box E2E coverage for ``ve-tos worm`` low-level APIs.
...
def worm_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_worm_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    worm_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-WORM-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in worm_leaf_commands:
    ...
    assert total_cases >= len(worm_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_worm_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    worm_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-WORM-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in worm_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_worm_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-WORM-LIVE get worm/object-lock config (may be empty)."""
    ...
    if get_result.exit_code != 0:
        skip_on_live_error("ve-tos worm get", get_result)
    envelope_assert.assert_success_envelope(get_result.require_envelope())


@pytest.mark.destructive
def test_worm_set_live_chain(
    cli_runner: CliRunner,
    temp_bucket,
) -> None:
    """@case-id LL-BC-WORM-CRUD set worm config on a fresh bucket.

    Note: ObjectLock/WORM cannot be enabled after bucket creation on standard
    buckets. We attempt and gracefully skip if the service rejects it.
    """
    from conftest import skip_on_live_error  # type: ignore[import-not-found]
    ...
    # Step 1: Set WORM config
    set_result = cli_runner.run([
        "ve-tos", ROOT, "set", "--bucket", temp_bucket,
        "--config", json.dumps(test_config, separators=(",", ":")),
    ])
    if set_result.exit_code != 0:
        # Expected: ObjectLock must be enabled at bucket creation time
