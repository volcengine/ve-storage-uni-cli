...
"""Black-box E2E coverage for ``ve-tos payment`` low-level APIs.
...
def payment_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_payment_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    payment_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-PAYMENT-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in payment_leaf_commands:
    ...
    assert total_cases >= len(payment_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_payment_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    payment_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-PAYMENT-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in payment_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_payment_set_get_restore_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-PAYMENT-LIVE get -> set -> get -> restore."""

    original_config = live_bucket_config.get_required(ROOT, e2e_bucket_name)

    try:
        live_bucket_config.set_config(ROOT, e2e_bucket_name, {"Payer": "BucketOwner"})
        ...
        assert get_result.payload()["body"]["payer"] == "BucketOwner"
    finally:
