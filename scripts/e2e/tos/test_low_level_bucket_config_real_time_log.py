...
"""Black-box E2E coverage for ``ve-tos real-time-log`` low-level APIs.
...
ROOT = "real-time-log"
# [Review Fix] CLI no longer requires --use-service-topic; dry-run succeeds now.
...
def real_time_log_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_real_time_log_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    real_time_log_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REAL_TIME_LOG-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in real_time_log_leaf_commands:
    ...
    assert total_cases >= len(real_time_log_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_real_time_log_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    real_time_log_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REAL_TIME_LOG-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in real_time_log_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_real_time_log_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-REAL_TIME_LOG-LIVE get real-time-log on reusable bucket."""
    ...
    if get_result.exit_code != 0:
        skip_on_live_error("ve-tos real-time-log get", get_result)
    envelope_assert.assert_success_envelope(get_result.require_envelope())


@pytest.mark.destructive
def test_real_time_log_set_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-REAL-TIME-LOG-CRUD set -> get -> delete -> get cycle."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]
    ...
    # real-time-log set requires the full API body via --config passthrough
    test_config = {
