...
"""Black-box E2E coverage for ``ve-tos cors`` low-level APIs.
...
def cors_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_cors_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    cors_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CORS-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in cors_leaf_commands:
    ...
    assert total_cases >= len(cors_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_cors_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    cors_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CORS-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in cors_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_cors_set_get_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-CORS-LIVE set -> get -> delete -> get with restoration."""
    ...
    try:
        ...
        rule = get_result.payload()["body"]["cors_rules"][0]
        assert rule["allowed_origins"] == ["https://example.com"]
        assert rule["allowed_methods"] == ["GET"]
        assert rule["allowed_headers"] == ["*"]
        assert rule["expose_headers"] == ["ETag"]
        assert rule["max_age_seconds"] == 60
