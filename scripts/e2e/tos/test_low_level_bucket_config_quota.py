...
"""Black-box E2E coverage for ``ve-tos quota`` low-level APIs.
...
def quota_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_quota_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    quota_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-QUOTA-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in quota_leaf_commands:
    ...
    assert total_cases >= len(quota_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_quota_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    quota_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-QUOTA-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in quota_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_quota_set_get_live_chain(
    cli_runner: CliRunner,
    temp_bucket: str,
) -> None:
    """@case-id LL-BC-QUOTA-LIVE isolated bucket quota set -> get -> remove -> verify."""

    try:
        # --- Step 1: set quota to 1 GiB ---
        set_result = cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"StorageQuota":1073741824}']
        )
        if set_result.exit_code != 0:
            skip_on_live_error("ve-tos quota set", set_result)
        envelope_assert.assert_success_envelope(set_result.require_envelope())

        # --- Step 2: get and verify quota equals 1073741824 ---
        get_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_result.exit_code != 0:
            skip_on_live_error("ve-tos quota get", get_result)
        envelope_assert.assert_success_envelope(get_result.require_envelope())
        body = get_result.payload()["body"]
        quota_value = body.get("StorageQuota") or body.get("storage_quota")
        assert quota_value == 1073741824, f"expected StorageQuota=1073741824, got {quota_value!r}"

        # --- Step 3: remove quota (set to 0) ---
        set_zero_result = cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"StorageQuota":0}']
        )
        if set_zero_result.exit_code != 0:
            skip_on_live_error("ve-tos quota set (remove)", set_zero_result)
        envelope_assert.assert_success_envelope(set_zero_result.require_envelope())

        # --- Step 4: get and verify quota is 0 ---
        get_zero_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_zero_result.exit_code != 0:
            skip_on_live_error("ve-tos quota get (after remove)", get_zero_result)
        envelope_assert.assert_success_envelope(get_zero_result.require_envelope())
        zero_body = get_zero_result.payload()["body"]
        zero_quota = zero_body.get("StorageQuota") if "StorageQuota" in zero_body else zero_body.get("storage_quota")
        assert zero_quota == 0, f"expected StorageQuota=0 after removal, got {zero_quota!r}"
    finally:
        # Belt and suspenders: temp_bucket will be cleaned up, but reset quota just in case
        cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"StorageQuota":0}']
        )
