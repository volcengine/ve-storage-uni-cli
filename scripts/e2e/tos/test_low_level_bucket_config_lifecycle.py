...
"""Black-box E2E coverage for ``ve-tos lifecycle`` low-level APIs.
...
def lifecycle_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_lifecycle_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    lifecycle_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-LIFECYCLE-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in lifecycle_leaf_commands:
    ...
    assert total_cases >= len(lifecycle_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_lifecycle_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    lifecycle_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-LIFECYCLE-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in lifecycle_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_lifecycle_set_get_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-LIFECYCLE-LIVE set -> get -> delete -> get with restoration."""

    original_config = live_bucket_config.get_optional(ROOT, e2e_bucket_name)
    test_config = {
        "Rules": [
            {
                "ID": "ve-tos-cli-e2e-lifecycle",
                "Status": "Enabled",
                "Prefix": "ve-tos-cli-e2e-lifecycle/",
                "Expiration": {"Days": 1},
            }
        ]
    }
    ...
    try:
        ...
        assert rule["id"] == "ve-tos-cli-e2e-lifecycle"
        assert rule["status"] == "Enabled"
        assert rule["prefix"] == "ve-tos-cli-e2e-lifecycle/"
