...
"""Black-box E2E coverage for ``ve-tos mrap`` low-level APIs.
...
def mrap_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_mrap_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    mrap_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-MRAP-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in mrap_leaf_commands:
    ...
    assert total_cases >= len(mrap_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_mrap_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    mrap_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-MRAP-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in mrap_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_mrap_control_plane_list_live_chain(
    cli_runner: CliRunner,
) -> None:
    """@case-id LL-ADV-MRAP-LIVE control endpoint list probe."""

    account_id = os.environ.get("TOS_ACCOUNT_ID")
    if not account_id:
        pytest.skip("TOS_ACCOUNT_ID not set; control-plane commands require account ID")

    result = cli_runner.run(["ve-tos", 'mrap', 'list', "--account-id", account_id, "--output", "json"])
    if result.exit_code != 0:
        skip_on_live_error("ve-tos mrap list", result)
    envelope_assert.assert_success_envelope(result.require_envelope())
    assert isinstance(result.payload(), dict)


@pytest.mark.destructive
def test_mrap_create_get_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-ADV-MRAP-CRUD create -> get -> list -> set-policy -> delete cycle."""
    ...
    mrap_name = f"e2emrap{e2e_bucket_name[:20].replace('-', '')}"

    create_config = {
        "Name": mrap_name,
        "Regions": [
            {"Bucket": e2e_bucket_name, "Region": "cn-beijing"},
        ],
    }
    ...
    try:
        ...
        del_result = cli_runner.run([
            ...
            "--name", mrap_name, "--force", "--confirm", f"tos://{mrap_name}",
        ])
