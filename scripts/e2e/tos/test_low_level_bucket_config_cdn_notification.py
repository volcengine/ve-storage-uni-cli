...
"""Black-box E2E coverage for ``ve-tos cdn-notification`` low-level APIs.
...
def cdn_notification_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_cdn_notification_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    cdn_notification_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CDN_NOTIFICATION-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in cdn_notification_leaf_commands:
    ...
    assert total_cases >= len(cdn_notification_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_cdn_notification_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    cdn_notification_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CDN_NOTIFICATION-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in cdn_notification_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_cdn_notification_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-CDN_NOTIFICATION-LIVE get cdn-notification on reusable bucket."""
    ...
    if get_result.exit_code == envelope_assert.EXIT_CODES["resource_not_found"]:
        envelope_assert.assert_failure_envelope(
            get_result.require_envelope(), expected_kind="resource_not_found"
        )
        return
    if get_result.exit_code != 0:
        skip_on_live_error("ve-tos cdn-notification get", get_result)
    envelope_assert.assert_success_envelope(get_result.require_envelope())


@pytest.mark.destructive
def test_cdn_notification_set_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-CDN-NOTIFICATION-CRUD set -> get -> delete -> get cycle."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]

    import json

    role = os.environ.get("TOS_CDN_NOTIFICATION_ROLE")
    if not role:
        pytest.skip("TOS_CDN_NOTIFICATION_ROLE not set; CDN notification set requires an existing role")
    custom_domain = os.environ.get("TOS_CDN_NOTIFICATION_DOMAIN")
    if not custom_domain:
        pytest.skip(
            "TOS_CDN_NOTIFICATION_DOMAIN not set; CDN notification set requires an existing CDN custom domain"
        )

    original_config = live_bucket_config.get_optional(ROOT, e2e_bucket_name)

    test_config = {
        # [Review Fix #CdnNotificationLivePrereq] The service validates that
        # Role and CustomDomain already exist, so live e2e must use account
        # resources instead of synthetic placeholder strings.
        "Role": role,
        "Rules": [
            {
                "RuleId": "e2e-cdn-notification",
                "CustomDomain": custom_domain,
                "Events": ["tos:ObjectCreated:*"],
                "Filter": {
                    "TOSKey": {
                        "FilterRules": [
                            {"Name": "prefix", "Value": "cdn-test/"}
                        ]
                    }
                },
            }
    ...
    try:
        ...
        if set_result.exit_code != 0:
            skip_on_live_error(f"ve-tos {ROOT} set", set_result)
        ...
        assert get_after.exit_code in (
            0,
            envelope_assert.EXIT_CODES["resource_not_found"],
        )
    finally:
