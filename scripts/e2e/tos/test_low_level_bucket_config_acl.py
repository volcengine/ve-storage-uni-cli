...
"""Black-box E2E coverage for ``ve-tos acl`` low-level APIs.
...
def acl_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_acl_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    acl_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ACL-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in acl_leaf_commands:
    ...
    assert total_cases >= len(acl_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_acl_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    acl_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-ACL-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in acl_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_acl_set_get_live_chain(
    cli_runner: CliRunner,
    temp_bucket: str,
) -> None:
    """@case-id LL-BC-ACL-LIVE isolated bucket acl set -> get -> restore."""

    # Save original ACL before modification
    original_get = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
    if original_get.exit_code != 0:
        skip_on_live_error("ve-tos acl get (original)", original_get)
    original_body = original_get.payload().get("body")

    try:
        # SET - apply private ACL
        set_result = cli_runner.run(["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--acl", "private"])
        if set_result.exit_code != 0:
            skip_on_live_error("ve-tos acl set", set_result)
        envelope_assert.assert_success_envelope(set_result.require_envelope())

        # GET - verify ACL was applied
        get_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_result.exit_code != 0:
            skip_on_live_error("ve-tos acl get", get_result)
        envelope_assert.assert_success_envelope(get_result.require_envelope())
        body = get_result.payload()["body"]
        assert "owner" in body or "grants" in body, f"expected owner or grants in body, got {body!r}"
        # Verify grant info reflects private ACL
        grants = body.get("grants") or body.get("Grants") or []
        if grants:
            for grant in grants:
                permission = grant.get("permission") or grant.get("Permission") or ""
                assert permission, f"expected non-empty permission in grant, got {grant!r}"
    finally:
        try:
            # Restore original ACL (best-effort on isolated bucket)
            if original_body is not None:
                cli_runner.run(["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--acl", "private"])
        except Exception:
            pass
