...
"""Black-box E2E coverage for ``ve-tos replication`` low-level APIs.
...
def replication_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_replication_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    replication_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REPLICATION-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in replication_leaf_commands:
    ...
    assert total_cases >= len(replication_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_replication_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    replication_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REPLICATION-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in replication_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_replication_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-REPLICATION-LIVE get replication config (may be empty)."""
    ...
    if get_result.exit_code != 0:
        skip_on_live_error("ve-tos replication get", get_result)
    envelope_assert.assert_success_envelope(get_result.require_envelope())


@pytest.mark.destructive
def test_replication_set_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
    temp_bucket,
) -> None:
    """@case-id LL-BC-REPLICATION-CRUD set -> get -> delete -> get cycle."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]

    import json

    # Replication requires a destination bucket
    dest_bucket = temp_bucket

    test_config = {
        "Role": "ServiceRole",
        "Rules": [
            {
                "ID": "e2e-replication-rule",
                "Status": "Enabled",
                "PrefixSet": ["test-prefix/"],
                "Destination": {
                    "Bucket": dest_bucket,
