...
"""Black-box E2E coverage for ``ve-tos bucket`` low-level APIs.
...
def bucket_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_bucket_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    bucket_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-CORE-BUCKET-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in bucket_leaf_commands:
    ...
    assert total_cases >= len(bucket_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_bucket_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    bucket_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-CORE-BUCKET-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in bucket_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_bucket_reusable_bucket_metadata_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-CORE-BUCKET-LIVE head/stat/info/location/list on reusable bucket."""

    for action in ("head", "stat", "info", "location"):
        result = cli_runner.run(["ve-tos", ROOT, action, "--bucket", e2e_bucket_name])
        assert result.exit_code == 0, f"{action} failed: {result.stderr}"
        envelope_assert.assert_success_envelope(result.require_envelope())

    head_uri = cli_runner.run(["ve-tos", ROOT, "head", f"tos://{e2e_bucket_name}"])
    assert head_uri.exit_code == 0, f"head via URI failed: {head_uri.stderr}"
    envelope_assert.assert_success_envelope(head_uri.require_envelope())

    listed = cli_runner.run(["ve-tos", ROOT, "list"])
    assert listed.exit_code == 0, listed.stderr
    envelope_assert.assert_success_envelope(listed.require_envelope())


@pytest.mark.destructive
def test_bucket_create_delete_isolated_live_chain(
    cli_runner: CliRunner,
    fresh_bucket_name: str,
) -> None:
    """@case-id LL-CORE-BUCKET-CREATE-DELETE-LIVE create -> head -> stat -> delete."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]

    create = cli_runner.run(
        ["ve-tos", ROOT, "create", "--bucket", fresh_bucket_name,
    ...
    try:
        head = cli_runner.run(["ve-tos", ROOT, "head", "--bucket", fresh_bucket_name])
        assert head.exit_code == 0, head.stderr
        envelope_assert.assert_success_envelope(head.require_envelope())

        stat = cli_runner.run(["ve-tos", ROOT, "stat", "--bucket", fresh_bucket_name])
        assert stat.exit_code == 0, stat.stderr
        envelope_assert.assert_success_envelope(stat.require_envelope())

        info = cli_runner.run(["ve-tos", ROOT, "info", "--bucket", fresh_bucket_name])
        assert info.exit_code == 0, info.stderr
        envelope_assert.assert_success_envelope(info.require_envelope())

        location = cli_runner.run(["ve-tos", ROOT, "location", "--bucket", fresh_bucket_name])
        assert location.exit_code == 0, location.stderr
        envelope_assert.assert_success_envelope(location.require_envelope())
    finally:
        ...
        assert delete.exit_code in (0, envelope_assert.EXIT_CODES["resource_not_found"])
