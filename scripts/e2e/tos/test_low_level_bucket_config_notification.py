...
"""Black-box E2E coverage for ``ve-tos notification`` low-level APIs.
...
def notification_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_notification_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    notification_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-NOTIFICATION-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in notification_leaf_commands:
    ...
    assert total_cases >= len(notification_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_notification_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    notification_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-NOTIFICATION-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in notification_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_notification_set_get_restore_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-NOTIFICATION-LIVE get -> set -> get -> restore."""

    original_config = live_bucket_config.get_required(ROOT, e2e_bucket_name)
    test_config = {
        "Version": original_config.get("version", ""),
        "Rules": [],
    }
    ...
    try:
        ...
        live_bucket_config.restore_notification(e2e_bucket_name, original_config)
