...
"""Black-box E2E coverage for ``ve-tos rename`` low-level APIs.
...
def rename_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_rename_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    rename_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-RENAME-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in rename_leaf_commands:
    ...
    assert total_cases >= len(rename_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_rename_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    rename_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-RENAME-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in rename_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_rename_set_get_delete_live_chain(
    cli_runner: CliRunner,
    temp_bucket: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-RENAME-LIVE isolated bucket rename config set -> get -> delete."""

    try:
        # SET - enable rename
        try:
            live_bucket_config.set_config(ROOT, temp_bucket, {"RenameEnable": True})
        except AssertionError:
            result = cli_runner.run(
                ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"RenameEnable":true}']
            )
            if result.exit_code != 0:
                skip_on_live_error("ve-tos rename set", result)

        # GET - verify rename enabled
        get_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_result.exit_code != 0:
            skip_on_live_error("ve-tos rename get", get_result)
        envelope_assert.assert_success_envelope(get_result.require_envelope())
        body = get_result.payload()["body"]
        rename_enable = body.get("rename_enable") or body.get("RenameEnable")
        assert rename_enable is True, f"expected RenameEnable=true, got {rename_enable!r}"

        # DELETE - remove rename config
        delete_result = cli_runner.run(
            [
                "ve-tos", ROOT, "delete", "--bucket", temp_bucket,
                "--force", "--confirm", f"tos://{temp_bucket}",
            ]
        )
        if delete_result.exit_code != 0:
            skip_on_live_error("ve-tos rename delete", delete_result)
        envelope_assert.assert_success_envelope(delete_result.require_envelope())

        # GET after delete - verify removed
        get_after = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        assert get_after.exit_code in (
            0,
            envelope_assert.EXIT_CODES["resource_not_found"],
        ), get_after.stderr
        if get_after.exit_code == 0:
            after_body = get_after.payload()["body"]
            after_enable = after_body.get("rename_enable") or after_body.get("RenameEnable")
            assert after_enable is not True, (
                f"expected RenameEnable removed after delete, got {after_enable!r}"
            )
    finally:
        try:
            # Best-effort cleanup: delete rename config on isolated bucket
            cli_runner.run(
                [
                    "ve-tos", ROOT, "delete", "--bucket", temp_bucket,
                    "--force", "--confirm", f"tos://{temp_bucket}",
