...
"""Black-box E2E coverage for ``ve-tos ap`` low-level APIs.
...
def ap_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_ap_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    ap_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-AP-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in ap_leaf_commands:
    ...
    assert total_cases >= len(ap_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_ap_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    ap_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-ADV-AP-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in ap_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_ap_control_plane_list_live_chain(
    cli_runner: CliRunner,
) -> None:
    """@case-id LL-ADV-AP-LIVE control endpoint list probe."""

    account_id = os.environ.get("TOS_ACCOUNT_ID")
    if not account_id:
        pytest.skip("TOS_ACCOUNT_ID not set; control-plane commands require account ID")

    result = cli_runner.run(["ve-tos", 'ap', 'list', "--account-id", account_id, "--output", "json"])
    if result.exit_code != 0:
        skip_on_live_error("ve-tos ap list", result)
    envelope_assert.assert_success_envelope(result.require_envelope())
    assert isinstance(result.payload(), dict)


@pytest.mark.destructive
def test_ap_create_get_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-ADV-AP-CRUD create -> get -> list -> set-policy -> delete cycle."""
    ...
    ap_name = f"e2eap{e2e_bucket_name[:20].replace('-', '')}"
    ...
    try:
        ...
        policy_config = {
            "Policy": json.dumps({
                "Statement": [{
                    ...
                    "Resource": [f"trn:tos::::{ap_name}/*"],
                }]
            }),
        }
        policy_result = cli_runner.run([
            "ve-tos", ROOT, "set-policy", "--account-id", account_id,
            "--name", ap_name,
            "--config", json.dumps(policy_config, separators=(",", ":")),
        ])
        if policy_result.exit_code == 0:
            envelope_assert.assert_success_envelope(policy_result.require_envelope())
        ...
        # [Review Fix #ApDeleteConfirm] AP deletion is a critical delete in
        # non-interactive e2e runs, so cleanup must use the same public URI
        # confirmation as the exercised command.
        cli_runner.run([
            "ve-tos", ROOT, "delete", "--account-id", account_id,
            "--name", ap_name,
            "--force",
