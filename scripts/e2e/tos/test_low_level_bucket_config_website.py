...
"""Black-box E2E coverage for ``ve-tos website`` low-level APIs.
...
def website_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_website_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    website_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-WEBSITE-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in website_leaf_commands:
    ...
    assert total_cases >= len(website_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_website_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    website_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-WEBSITE-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in website_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_website_set_get_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-WEBSITE-LIVE set -> get -> delete -> get with restoration."""
    ...
    try:
        ...
        assert body["index_document"]["suffix"] == "index.html"
        assert body["error_document"]["key"] == "error.html"
