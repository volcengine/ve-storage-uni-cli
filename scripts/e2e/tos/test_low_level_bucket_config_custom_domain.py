...
"""Black-box E2E coverage for ``ve-tos custom-domain`` low-level APIs.
...
def custom_domain_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_custom_domain_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    custom_domain_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CUSTOM_DOMAIN-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in custom_domain_leaf_commands:
    ...
    assert total_cases >= len(custom_domain_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_custom_domain_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    custom_domain_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-CUSTOM_DOMAIN-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in custom_domain_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_custom_domain_list_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-CUSTOM_DOMAIN-LIVE list custom domains on reusable bucket."""
    ...
    if list_result.exit_code != 0:
        skip_on_live_error("ve-tos custom-domain list", list_result)
    envelope_assert.assert_success_envelope(list_result.require_envelope())
    body = list_result.payload()["body"]
    assert body is not None, "expected non-null body from custom-domain list"


@pytest.mark.destructive
def test_custom_domain_set_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-CUSTOM-DOMAIN-CRUD set -> list -> delete cycle.

    Note: custom-domain set requires a real domain with CNAME pointing to bucket.
    We attempt the operation and gracefully skip if the service rejects it.
    """
    from conftest import skip_on_live_error  # type: ignore[import-not-found]

    import json

    # Use a fake domain — this will likely fail with a validation error on the
    # service side, but we exercise the full CLI path and capture the error.
    test_domain = f"e2e-test-{e2e_bucket_name[:16]}.example.com"
    test_config = {
        "CustomDomainRule": {
            "Domain": test_domain,
        }
    ...
    if set_result.exit_code != 0:
        # Expected: service will reject fake domain
    ...
    try:
        ...
        del_result = cli_runner.run([
            ...
            "--domain", test_domain, "--force", "--confirm", f"tos://{e2e_bucket_name}",
        ])
