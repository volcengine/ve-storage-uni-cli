...
"""Black-box E2E coverage for ``ve-tos intelligent-tiering`` low-level APIs.
...
def intelligent_tiering_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_intelligent_tiering_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    intelligent_tiering_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-INTELLIGENT_TIERING-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in intelligent_tiering_leaf_commands:
    ...
    assert total_cases >= len(intelligent_tiering_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_intelligent_tiering_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    intelligent_tiering_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-INTELLIGENT_TIERING-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in intelligent_tiering_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_intelligent_tiering_set_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-INTELLIGENT_TIERING-LIVE set -> get -> update -> verify with restoration."""
    ...
    try:
        ...
        assert status == "Enabled", f"expected Enabled, got {status!r}"
        transitions = body.get("transitions") or body.get("Transitions")
        assert isinstance(transitions, list), f"expected Transitions array, got {transitions!r}"
        assert len(transitions) >= 1, f"expected at least 1 transition, got {transitions!r}"
        first_days = transitions[0].get("days") or transitions[0].get("Days")
        assert first_days == 90, f"expected Days=90, got {first_days!r}"

        # NOTE: TOS intelligent-tiering does not support in-place rule updates (returns ExistRule).
        # Only set→get verification is performed.
    finally:
