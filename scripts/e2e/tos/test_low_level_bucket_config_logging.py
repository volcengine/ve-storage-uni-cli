...
"""Black-box E2E coverage for ``ve-tos logging`` low-level APIs.
...
def logging_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_logging_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    logging_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-LOGGING-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in logging_leaf_commands:
    ...
    assert total_cases >= len(logging_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_logging_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    logging_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-LOGGING-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in logging_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_logging_disable_get_restore_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-LOGGING-LIVE get -> disable -> get with safe restoration."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]

    original_config = live_bucket_config.get_optional(
        ROOT,
        e2e_bucket_name,
        empty_field="logging_enabled",
    )
    if original_config is not None:
        pytest.skip("logging restore schema is environment-specific; skip to avoid pollution")

    try:
        # SET - disable logging
        live_bucket_config.set_config(ROOT, e2e_bucket_name, {})

        # GET - verify logging is disabled
        ...
        assert body.get("logging_enabled") is None, (
            f"expected logging_enabled to be absent after disable, got {body!r}"
        )
    finally:
        try:
