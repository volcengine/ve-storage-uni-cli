...
"""Black-box E2E coverage for ``ve-tos pay-by-traffic`` low-level APIs.
...
def pay_by_traffic_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_pay_by_traffic_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    pay_by_traffic_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-PAY_BY_TRAFFIC-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in pay_by_traffic_leaf_commands:
    ...
    assert total_cases >= len(pay_by_traffic_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_pay_by_traffic_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    pay_by_traffic_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-PAY_BY_TRAFFIC-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in pay_by_traffic_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_pay_by_traffic_set_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-PAY_BY_TRAFFIC-LIVE get -> set -> get with restoration."""
    ...
    if get_result.exit_code != 0:
        skip_on_live_error("ve-tos pay-by-traffic get", get_result)
    envelope_assert.assert_success_envelope(get_result.require_envelope())
    original_body = get_result.payload()["body"]
    original_charge_type = original_body.get("charge_type") or original_body.get("ChargeType")
    assert original_charge_type is not None, f"expected charge_type in body, got {original_body!r}"

    try:
        # SET - apply PayByTraffic
        set_result = cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", e2e_bucket_name, "--config",
             '{"ChargeType":"PayByTraffic"}']
        )
        if set_result.exit_code != 0:
            skip_on_live_error("ve-tos pay-by-traffic set", set_result)
        envelope_assert.assert_success_envelope(set_result.require_envelope())

        # GET - verify ChargeType was applied
        verify_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", e2e_bucket_name])
        if verify_result.exit_code != 0:
            skip_on_live_error("ve-tos pay-by-traffic get (verify)", verify_result)
        envelope_assert.assert_success_envelope(verify_result.require_envelope())
        verify_body = verify_result.payload()["body"]
        charge_type = verify_body.get("charge_type") or verify_body.get("ChargeType")
        assert charge_type == "PayByTraffic", (
            f"expected ChargeType=PayByTraffic, got {charge_type!r}"
        )
    finally:
        try:
            # Restore original ChargeType
            live_bucket_config.set_config(
                ROOT, e2e_bucket_name, {"ChargeType": original_charge_type}
            )
        except Exception:
            pass
