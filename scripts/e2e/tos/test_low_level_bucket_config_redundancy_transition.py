...
"""Black-box E2E coverage for ``ve-tos redundancy-transition`` low-level APIs.
...
def redundancy_transition_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_redundancy_transition_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    redundancy_transition_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REDUNDANCY_TRANSITION-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in redundancy_transition_leaf_commands:
    ...
    assert total_cases >= len(redundancy_transition_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_redundancy_transition_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    redundancy_transition_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-REDUNDANCY_TRANSITION-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in redundancy_transition_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_redundancy_transition_list_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-REDUNDANCY_TRANSITION-LIVE list redundancy-transition tasks."""
    ...
    if list_result.exit_code != 0:
        skip_on_live_error("ve-tos redundancy-transition list", list_result)
    envelope_assert.assert_success_envelope(list_result.require_envelope())


@pytest.mark.destructive
def test_redundancy_transition_create_get_delete_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
) -> None:
    """@case-id LL-BC-REDUNDANCY-TRANSITION-CRUD create -> get -> list -> delete cycle."""
    from conftest import skip_on_live_error  # type: ignore[import-not-found]
    ...
    # Step 1: Create transition task
    create_result = cli_runner.run([
        "ve-tos", ROOT, "create", "--bucket", e2e_bucket_name,
    ...
    try:
        ...
        if task_id:
            ...
            get_result = cli_runner.run([
                ...
                "--x-tos-redundancy-transition-taskid", task_id,
            ])
            ...
            remaining_result = cli_runner.run([
                "ve-tos", ROOT, "get-remaining-time", "--bucket", e2e_bucket_name,
                "--estimatedRemainingTime", "",
                "--x-tos-redundancy-transition-taskid", task_id,
            ])
            # This may fail if the feature is not fully supported; don't block
            if remaining_result.exit_code == 0:
                envelope_assert.assert_success_envelope(remaining_result.require_envelope())

        # Step 4: List tasks
        list_result = cli_runner.run(["ve-tos", ROOT, "list", "--bucket", e2e_bucket_name])
        if list_result.exit_code != 0:
