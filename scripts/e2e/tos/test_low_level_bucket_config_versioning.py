...
"""Black-box E2E coverage for ``ve-tos versioning`` low-level APIs.
...
def versioning_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_versioning_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    versioning_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-VERSIONING-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in versioning_leaf_commands:
    ...
    assert total_cases >= len(versioning_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_versioning_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    versioning_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-VERSIONING-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in versioning_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])

@pytest.mark.destructive
def test_versioning_set_get_live_chain(
    cli_runner: CliRunner,
    temp_bucket: str,
) -> None:
    """@case-id LL-BC-VERSIONING-LIVE isolated bucket versioning set -> get -> suspend."""

    # GET original status
    original_get = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
    if original_get.exit_code != 0:
        skip_on_live_error("ve-tos versioning get (original)", original_get)
    original_status = original_get.payload()["body"].get("status")

    try:
        # SET - enable versioning
        set_result = cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"Status":"Enabled"}']
        )
        if set_result.exit_code != 0:
            skip_on_live_error("ve-tos versioning set (Enabled)", set_result)
        envelope_assert.assert_success_envelope(set_result.require_envelope())

        # GET - verify Enabled
        get_result = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_result.exit_code != 0:
            skip_on_live_error("ve-tos versioning get", get_result)
        ...
        # SET - suspend versioning
        suspend_result = cli_runner.run(
            ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"Status":"Suspended"}']
        )
        if suspend_result.exit_code != 0:
            skip_on_live_error("ve-tos versioning set (Suspended)", suspend_result)
        envelope_assert.assert_success_envelope(suspend_result.require_envelope())

        # GET - verify Suspended
        get_suspended = cli_runner.run(["ve-tos", ROOT, "get", "--bucket", temp_bucket])
        if get_suspended.exit_code != 0:
            skip_on_live_error("ve-tos versioning get (after suspend)", get_suspended)
        envelope_assert.assert_success_envelope(get_suspended.require_envelope())
        suspended_body = get_suspended.payload()["body"]
        suspended_status = suspended_body.get("status") or suspended_body.get("Status")
        assert suspended_status == "Suspended", f"expected Status=Suspended, got {suspended_status!r}"
    finally:
        try:
            # Versioning once Enabled can only be Suspended, never disabled.
            # Restore to Suspended as the safest final state.
            if original_status in (None, "", "Suspended"):
                cli_runner.run(
                    ["ve-tos", ROOT, "set", "--bucket", temp_bucket, "--config", '{"Status":"Suspended"}']
                )
        except Exception:
