...
"""Black-box E2E coverage for ``ve-tos max-age`` low-level APIs.
...
def max_age_leaf_commands(cli_runner: CliRunner) -> list[dict[str, Any]]:
...
@pytest.mark.slow
def test_max_age_all_leaf_commands_exercise_full_parameter_surface_dry_run(
    cli_runner: CliRunner,
    max_age_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-MAX_AGE-SURFACE every leaf operation and parameter is asserted."""

    failures: list[str] = []
    total_cases = 0

    for leaf in max_age_leaf_commands:
    ...
    assert total_cases >= len(max_age_leaf_commands)
    assert not failures, "\n".join(failures[:80])


@pytest.mark.slow
def test_max_age_leaf_operations_chain_in_metadata_order_dry_run(
    cli_runner: CliRunner,
    max_age_leaf_commands: list[dict[str, Any]],
    tmp_path: Path,
) -> None:
    """@case-id LL-BC-MAX_AGE-CHAIN all operations for this root are asserted in one flow."""
    ...
    expected_commands = {str(leaf["command"]) for leaf in max_age_leaf_commands}
    assert set(seen_commands) == expected_commands
    assert not failures, "\n".join(failures[:80])


@pytest.mark.destructive
def test_max_age_set_get_delete_get_live_chain(
    cli_runner: CliRunner,
    e2e_bucket_name: str,
    live_bucket_config,
) -> None:
    """@case-id LL-BC-MAX-AGE-LIVE set -> get -> delete -> get with restoration."""

    original_config = live_bucket_config.get_optional(ROOT, e2e_bucket_name)
    test_config = {"MaxAge": 60}
    ...
    try:
        ...
        assert get_result.payload()["body"]["max_age"] == 60
